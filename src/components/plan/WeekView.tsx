...
// Hard types that should never be back-to-back
...
function getValidationWarnings(workouts: Workout[], movedId: string, targetDate: string): ValidationWarning[] {
  ...
  for (let i = 1; i < sorted.length; i++) {
    ...
    const curr = sorted[i]
    const prevDate = parseISO(prev.scheduled_date)
    const currDate = parseISO(curr.scheduled_date)
    ...
    if (dayDiff === 1) {
      ...
      if ((prev.type === 'long' && HARD_TYPES.has(curr.type)) ||
          (HARD_TYPES.has(prev.type) && curr.type === 'long')) {
...
export default function WeekView({
  ...
  async function toggleComplete(workout: Workout) {
    ...
    setCompletedIds(prev => {
      ...
      nowComplete ? next.add(workout.id) : next.delete(workout.id)
      return next
    })
    ...
    try {
      ...
      setCompletedIds(prev => {
        const next = new Set(prev)
        nowComplete ? next.delete(workout.id) : next.add(workout.id)
        return next
      })
    } finally {
      setLoadingId(null)
  ...
  function handleDragOver(e: React.DragEvent, targetWorkout: Workout) {
    ...
    if (draggingId && draggingId !== targetWorkout.id) {
      ...
      const w = getValidationWarnings(localWorkouts, draggingId, targetWorkout.scheduled_date)
      setWarnings(w)
    }
  }
  ...
  async function handleDrop(e: React.DragEvent, targetWorkout: Workout) {
    ...
    // Optimistic update — swap dates in local state immediately
    setLocalWorkouts(prev => prev.map(w => {
      if (w.id === sourceId) return {
        ...w,
        scheduled_date: targetWorkout.scheduled_date,
        day_of_week: targetWorkout.day_of_week,
    ...
    try {
      const res = await fetch('/api/workouts/move', {
        ...
        body: JSON.stringify({
          workoutId: sourceId,
          newDate: targetWorkout.scheduled_date,
          newWeekNumber: targetWorkout.week_number,
          newDayOfWeek: targetWorkout.day_of_week,
        }),
      })
  ...
  // ─── Cross-week move (mobile / manual) ───────────────────────────────────
  async function confirmMove() {
    if (!showMoveSheet || !moveDate) return
    const workout = showMoveSheet.workout
    setShowMoveSheet(null)

    // Calculate week number and day of week from the chosen date
    const chosen = parseISO(moveDate)
    const dayOfWeek = (chosen.getDay() + 6) % 7 // 0=Mon…6=Sun
    ...
    try {
      await fetch('/api/workouts/move', {
        ...
        body: JSON.stringify({
          ...
          // week_number calculated server-side isn't ideal but close enough for now
          newWeekNumber: workout.week_number,
        }),
      })
    } catch {
      // silent fail — router.refresh will correct state
    } finally {
      router.refresh()
    }
  }

  const sorted = [...localWorkouts].sort((a, b) =>
    ...
    <div>
      ...
      {warnings.length > 0 && (
        <div className="mb-3 rounded-xl px-3 py-2 flex items-start gap-2"
          style={{ backgroundColor: '#F97316' + '22', border: '1px solid #F97316' + '44' }}>
          <AlertTriangle size={14} className="mt-0.5 shrink-0" style={{ color: '#F97316' }} />
          <div>
            {warnings.map((w, i) => (
              <p key={i} className="text-xs" style={{ color: '#F97316' }}>{w.message}</p>
            ))}
            <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>
              You can still drop here — just a heads up.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="space-y-2">
        {sorted.map(workout => {
          ...
          const paceDisplay = formatWorkoutPace(
            workout.pace_min_seconds,
            workout.pace_max_seconds,
            workout.pace_target
          )
          ...
          return (
            ...
            >
              {/* Left colour bar — orange if skipped */}
              <div className="w-1 shrink-0"
                style={{ backgroundColor: isSkipped ? '#F97316' : colour }} />
                ...
                <Link href={`/workout/${workout.id}`} className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <p className="text-[10px] font-bold uppercase tracking-wide"
                      style={{ color: isSkipped ? '#F97316' : colour }}>
                      {TYPE_LABELS[workout.type] ?? workout.type}
                    </p>
                    {isSkipped && (
                      <span className="text-[9px] font-bold uppercase tracking-wide px-1.5 py-0.5 rounded-full"
                        style={{ backgroundColor: '#F97316' + '33', color: '#F97316' }}>
                        SKIPPED
                      </span>
                    )}
                  </div>
                      ...
                      {openMenuId === workout.id && (
                        ...
                        >
                          ...
                          <button
                            onClick={() => {
                              setOpenMenuId(null)
                              setMoveDate(workout.scheduled_date)
                              setShowMoveSheet({ workout })
                            }}
                            className="w-full flex items-center gap-2 px-3 py-2 text-sm text-left hover:opacity-80"
